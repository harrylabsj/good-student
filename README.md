...
`packages/workbuddy/` 提供 WorkBuddy 的 Skill、CLI 连接器、错题教练专家包和 Buddy 应用配置
草案，采用「专家 + 连接器」结构：专家依赖同名连接器，召唤时平台自动引导连接。连接器为
CLI + Skill 方案，`cli.json` 声明平台托管 Python 运行时并直接安装包内置 wheel——
**无需发布 PyPI、无需 uvx**。按 `docs/workbuddy-publishing-plan.md` 完成本地预览与平台提交。
